...
void ChessboardCornersMatch(std::string& result_save_path, std::vector<cv::Mat>& chessboards_total_mid, std::vector<cv::Mat>& chessboards_total_right,
						...
						cv::Mat img1, cv::Mat img2, cv::Size board_size)
{
	...
	cv::imwrite(result_save_path, SmallMat);
						...
						std::vector<cv::Mat>& chessboards)
{
	...
	for(int num = 0; num < image_points_ex.size(); num++)
	{
		...
		bool dist1, dist2, dist3, dist4;
		dist1 = SortByXY(bound_point1, bound_point2, bound_point3, bound_point4);
		dist2 = SortByXY(bound_point2, bound_point1, bound_point3, bound_point4);
		dist3 = SortByXY(bound_point3, bound_point1, bound_point2, bound_point4);
		dist4 = SortByXY(bound_point4, bound_point1, bound_point2, bound_point3);
...
{	
	...
	for( int i = 0; i < nimages; i++ ){
		...
		std::cout << "start calib " << i<<" images"<<std::endl;
	...
	fout << "reprojection error: " << err_first << " pixel" << std::endl << std::endl;
	std::cout << "save intrinsic calib result ..."<<std::endl<< std::endl << std::endl;
	fout << "mat_k:" << std::endl;
	fout << camera_matrix.at<double>(0, 0)<<", "<< camera_matrix.at<double>(0, 1)<<", "<<camera_matrix.at<double>(0, 2)<<","<<std::endl
		<< camera_matrix.at<double>(1, 0)<<", "<< camera_matrix.at<double>(1, 1)<<", "<<camera_matrix.at<double>(1, 2)<<","<<std::endl
		<< camera_matrix.at<double>(2, 0)<<", "<< camera_matrix.at<double>(2, 1)<<", "<<camera_matrix.at<double>(2, 2)<<std::endl;
	fout << "mat_d:"<<std::endl;
	fout << dist_coeffs.at<double>(0, 0)<<", "<<dist_coeffs.at<double>(0, 1)<<", "<<dist_coeffs.at<double>(0, 2) <<", "
		<< dist_coeffs.at<double>(0, 3)<<", "<<dist_coeffs.at<double>(0, 4)<< std::endl<< std::endl;
	
	if( err_first > 0.6){
		std::cout<<" intrisic calib result is wrong !"<<std::endl;
		return false;
	}
				...
				std::vector<cv::Mat>& chessboards_total_right, std::ofstream& fout, cv::Mat& T_out)
{
	...
	cv::Mat img_s1 = cv::imread(input_path_mid,0);
	cv::Mat img_s2 = cv::imread(input_path_right,0);
	...
	if(rms > 1){
		std::cout << "extrisic calib result is wrong !" <<std::endl;
		return false;
	}
	fout << "mat_rt:" << std::endl;
	T_out = (cv::Mat_<double>(4,4)<< R.at<double>(0, 0), R.at<double>(0, 1), R.at<double>(0, 2), T.at<double>(0, 0),
				R.at<double>(1, 0), R.at<double>(1, 1), R.at<double>(1, 2), T.at<double>(1, 0),
				R.at<double>(2, 0), R.at<double>(2, 1), R.at<double>(2, 2), T.at<double>(2, 0),
				0, 0, 0, 1);
	fout << R.at<double>(0, 0)<<", "<<R.at<double>(0, 1)<<", "<<R.at<double>(0, 2)<<", "<<T.at<double>(0, 0)<<","<< std::endl
		<< R.at<double>(1, 0)<<", "<<R.at<double>(1, 1)<<", "<<R.at<double>(1, 2)<<", "<<T.at<double>(1, 0)<<","<< std::endl
		<< R.at<double>(2, 0)<<", "<<R.at<double>(2, 1)<<", "<<R.at<double>(2, 2)<<", "<<T.at<double>(2, 0)<<","<< std::endl
		<< 0 <<", "<< 0 <<", "<< 0 <<", "<< 1 <<std::endl;
	return true;
}
