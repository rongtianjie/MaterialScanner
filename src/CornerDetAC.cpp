...
void CornerDetAC::createkernel(float angle1, float angle2, int kernelSize, cv::Mat &kernelA, cv::Mat &kernelB, cv::Mat &kernelC, cv::Mat &kernelD)
{
	...
	// std::cout << "kernelA:" << kernelA << std::endl << "kernelB:" << kernelB << std::endl
	// 	<< "kernelC:" << kernelC<< std::endl << "kernelD:" << kernelD << std::endl;
	
	kernelA = kernelA / cv::sum(kernelA)[0]; //sum[0] is the sum of the 1st channel
...
void CornerDetAC::nonMaximumSuppression(cv::Mat& inputCorners, vector<cv::Point2f>& outputCorners, int patchSize, dtype threshold, int margin)
{
...
void CornerDetAC::refineCorners(vector<cv::Point2f> &cornors, cv::Mat imgDu, cv::Mat imgDv, cv::Mat imgAngle, cv::Mat imgWeight, float radius)
{
	// image dimensions
	...
	//	continue;
		...
		for (int u = startX; u < startX + ROIwidth; u++)
			for (int v = startY; v < startY + ROIheight; v++)
			{
				...
				if (no < 0.1)
					continue;
                float A=imgDu.at<dtype>(v, u)/no;
                float B=imgDv.at<dtype>(v, u)/no;
				o.x=A;
				o.y=B;
...
void CornerDetAC::cornerCorrelationScore(cv::Mat img, cv::Mat imgWeight, vector<cv::Point2f> cornersEdge, float &score)
{
	//center
...
//score corners
void CornerDetAC::scoreCorners(cv::Mat img, cv::Mat imgAngle, cv::Mat imgWeight, vector<cv::Point2f> &cornors, vector<int> radius, vector<float> &score)
{
	//for all corners do
	for (int i = 0; i < cornors.size(); i++)
	{
...
void CornerDetAC::detectCorners(cv::Mat &Src, Corners& mcorners,  dtype scoreThreshold, bool isrefine)
{
	...
	for (int i = 0; i < 6; i++) 
	{
		...
		//std::cout << "kernelA:" << kernelA1 << std::endl << "kernelB:" << kernelB1 << std::endl
			//<< "kernelC:" << kernelC1 << std::endl << "kernelD:" << kernelD1 << std::endl;
